...
- Added an integrated disposable authorization test in `26c122e` that pairs a device, registers its context, resolves the list, and successfully captures an item (`201 Created`) using an in-memory database. This confirms the exact pair-then-capture flow without modifying family-live.
